...
class Message:
    ...
    DURATION_ERROR = (
        f"{Prefix.RESPONSE_PREFIX} 영상의 길이를 가져오는 중 오류가 발생했습니다"
    )
    NOT_FOUND = f"{Prefix.RESPONSE_PREFIX} 영상을 찾을 수 없습니다: {{}}"
    REQUEST_SUCCESS = f"{Prefix.RESPONSE_PREFIX} {{}} (신청: {{}}/{{}}회)"
    REQUEST_BANNED_KEYWORD = (
        f"{Prefix.RESPONSE_PREFIX} 영상 제목에 금지어가 포함되어 있습니다: {{}}"
    )
    REQUEST_BANNED_CHANNEL = (
        f"{Prefix.RESPONSE_PREFIX} 유튜브 채널명이 금지되어 있습니다: {{}}"
    )
    ...
    REQUEST_LIMIT_REACHED = (
        f"{Prefix.RESPONSE_PREFIX} {{}}님의 신청 횟수가 초과되었습니다 (최대 {{}}회)"
    )
    ...
    IS_NOT_OWN_VIDEO = (
        f"{Prefix.RESPONSE_PREFIX} {{}}님만 현재 영상을 스킵할 수 있습니다"
    )
